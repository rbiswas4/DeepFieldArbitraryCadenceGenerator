...
C
      WRITE (6,'(A)') 'What are we doing?  Initial(I) or cleanup(C)?'
      READ (5,'(A)') LINE
      ...
      ELSEIF (LINE(1:1).EQ.'C') THEN
        CLEAN = .TRUE.
      ELSE
        WRITE (6,*) 'LSSTDFACG:  Invalid response: ',LINE
        STOP
      ENDIF
...
C  Get file name
C
      WRITE (6,'(A)') 'What Arbitrary Cadence file to process?'
      READ (5,'(A)') DFACGF
...
C Open OpsimObs output.  Ask the name.
        WRITE (6,'(A)') 'What OpsimObs output file to process?'
        READ (5,'(A)') OPSOBO
